...
export const getHighLightTextHTML = ({
  ...
  const chunks: HighLightTextHTMLChunk[] = findAll({
    sourceString,
    searchWords,
    ...option,
  });
...
export const isSemiIcon = (icon: any): boolean =>
  React.isValidElement(icon) && get(icon.type, "elementType") === "Icon";
...
export function getFocusableElements(node: HTMLElement) {
  ...
  const focusableElements = Array.from(
    node.querySelectorAll<HTMLElement>(focusableSelectorsStr)
  );
