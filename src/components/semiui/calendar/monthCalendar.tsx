...
> {
  ...
  calcItemLimit = () => {
    this.contentCellHeight =
      this.cellDom.current.getBoundingClientRect().height;
    return Math.max(
      0,
      Math.ceil((this.contentCellHeight - contentPadding) / contentHeight)
    );
  };
  ...
  componentDidUpdate(
    prevProps: MonthCalendarProps,
    prevState: MonthCalendarState
  ) {
    ...
    if (!isEqual(prevEventKeys, nowEventKeys) || itemLimitUpdate) {
      this.foundation.parseMonthlyEvents(
        (itemLimit || this.props.events) as any
      );
    }
  }
  ...
  renderHeader = (dateFnsLocale: Locale["dateFnsLocale"]) => {
    ...
    this.monthlyData = this.foundation.getMonthlyData(
      displayValue,
      dateFnsLocale
    );
    return (
        ...
        <div className={`${cardCls}-content`}>
          ...
          <div className={`${cardCls}-body`}>
            <ul className={`${cardCls}-list`}>
              {events.map((item) => (
                <li key={item.key || `${item.start.toString()}-event`}>
                  {item.children}
                </li>
              ))}
            </ul>
        ...
        <LocaleConsumer componentName="Calendar">
          {(locale: Locale["Calendar"], localeCode: string) => (
            <span className={`${prefixCls}-date`}>
              {month}
              <span className={`${cssClasses.PREFIX}-today-date`}>
                &nbsp;{date}
              </span>
              {locale.datestring}
            </span>
      ...
      >
        <ul role="row" className={`${prefixCls}-skeleton`}>
          {weekDay.map((each) => {
            ...
            const shouldRenderCollapsed = Boolean(
              day && day[ind] && day[ind].length > itemLimit
            );
            const inner = (
              <li
                role="gridcell"
      ...
      <div role="presentation" className={`${prefixCls}-week`}>
        <ul role="presentation" className={`${prefixCls}-grid-col`}>
          {Object.keys(this.monthlyData).map((weekInd) =>
            this.renderWeekRow(
              weekInd,
              this.monthlyData[weekInd],
              parsedEvents[weekInd]
            )
          )}
        </ul>
      </div>
