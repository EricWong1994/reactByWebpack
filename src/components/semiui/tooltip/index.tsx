...
import {
  ...
  DOMRectLikeType,
} from "@douyinfe/semi-foundation/utils/dom";
...
import {
  ...
  cssClasses,
  numbers,
} from "@douyinfe/semi-foundation/tooltip/constants";
...
export default class Tooltip extends BaseComponent<TooltipProps, TooltipState> {
  ...
  get adapter(): TooltipAdapter<TooltipProps, TooltipState> {
    return {
      ...
      insertPortal: (
        ...
        { position, ...containerStyle }: { position: Position }
      ) => {
        this.setState(
          {
            isInsert: true,
      ...
      getPopupContainerRect: () => {
        ...
        if (container && isHTMLElement(container)) {
          const boundingRect: DOMRectLikeType = convertDOMRectToObject(
      ...
      registerClickOutsideHandler: (cb: () => void) => {
        ...
        this.clickOutsideHandler = (e: React.MouseEvent): any => {
          ...
          popupEl = ReactDOM.findDOMNode(
            popupEl as React.ReactInstance
          ) as HTMLDivElement;
          if (
            (el &&
              !(el as any).contains(e.target) &&
              popupEl &&
      ...
      registerScrollHandler: (
        rePositionCb: (arg: { x: number; y: number }) => void
      ) => {
      ...
      getTriggerNode: () => {
        ...
        if (!isHTMLElement(this.triggerEl.current)) {
          triggerDOM = ReactDOM.findDOMNode(
            this.triggerEl.current as React.ReactInstance
          );
        }
  ...
  renderPortal = () => {
    ...
    const { prefixCls, content, showArrow, style, motion, role, zIndex } =
      this.props;
