...
class UseVirtualTable extends PureComponent {
	...
	state = {
		...
		rowSize: [0, 20], // 可视区域显示的行号
		// colSize: [0, 0], // 可视区域显示的列号
		colSize: [0, 100], // 可视区域显示的列号
	};
		...
		dataSource.forEach((item, i) => {
			...
			if (currentStep === 0) {
				if (totalHeight >= scrollTop - OFFSET_VERTICAL) {
					...
					console.log('rowSize[0]: ', rowSize[0]);
					rowSize[0] = i;
					currentStep += 1;
				} else {
					hiddenTopHeight += rowHeight;
					console.log('hiddenTopHeight: ', hiddenTopHeight);
				}
	...
	setHorizontalData = () => {
		...
		columns.forEach((item, i) => {
			...
			if (
				currentStep === 1 &&
				totalWidth > scrollLeft + width + OFFSET_HORIZONTAL
			) {
				// 计算出可视区域结束列号
				colSize[1] = i;
				currentStep += 1;
	...
	tableBodyRender = () => {
		...
		showCols.forEach((col, i) => {
			...
			cols.push(
				<col key={`${code}-${colSize[0] + i}`} width={`${width}px`} />
			);
		});
		if (colSize[1]) {
			...
			>
				<div
					...
					style={{
						...
						paddingTop: `${hiddenTopStyle?.height}`,
					}}
				>
					<table>
						<colgroup>{cols}</colgroup>
							...
							{showData.map((row, i) => {
								...
								return (
									<tr
										...
										key={index}
										data-rowindex={index}
										// ref={this.getTrRef.bind(null, index)}
									>
										{colSize[0] ? <td /> : null}
										{showCols.map(({ code }, j) => {
	...
	render() {
		...
		return (
			<div
				...
				ref={this.getVirtualTableRef}
				onScroll={this.handleHorizontalScroll}
			>
				<div
					className='use-virtual-table-body'
					ref={this.getHorizontalDom}
				>
				...
				<div
					...
					style={{
						height: `${
							this.virtualTableHeight -
							(this.tableHeader?.clientHeight || 34)
						}px`,
